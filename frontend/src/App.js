...
function App() {
  ...
  const MAX_TRAIL_LENGTH = 20; // Keep last 20 positions in the trail
  
  // Add state for simulation control and tracking
  const [rerouteAttempts, setRerouteAttempts] = useState(0);
  const [simulationSpeed, setSimulationSpeed] = useState(5); // Speed increment percentage per step
  ...
  const startDetection = async () => {
    try {
      ...
      const requestBody = {
        use_rtsp: true, // Default to RTSP
        // You can use a custom RTSP URL if needed:
        // rtsp_url: 'rtsp://username:password@camera_ip:port/stream',
        camera_id: 0 // Used if use_rtsp is false
      };
      ...
      if (response.data.status === 'success') {
        ...
        frameIntervalRef.current = setInterval(async () => {
          try {
            ...
            const detectedObjects = objectsResponse.data.objects || [];
  ...
  const processObstaclesFromDetection = (detectedObjects) => {
    ...
    obstacleObjects.forEach(obj => {
      ...
      const existingObstacle = obstacles.find(existing => {
      ...
      if (!existingObstacle) {
        ...
        if (simulationActive && path.length > 0) {
          // Check if this new obstacle affects the current path
          let pathAffected = false;
          
          // Check if obstacle is near the current location during simulation
          const distToObstacle = calculateDistance(currentLocation, position);
          const distInMeters = distToObstacle * 111000;
          
          // If obstacle is within 20 meters of current position, it affects the path
          if (distInMeters < 20) {
            pathAffected = true;
          } else {
            // Otherwise check if it intersects any part of the path
            for (let i = 0; i < path.length - 1; i++) {
              if (checkPathSegmentAgainstObstacles(path[i], path[i+1], [newObstacle])) {
                pathAffected = true;
                break;
              }
            }
          ...
          if (pathAffected) {
            console.log(`New obstacle affects current path - triggering rerouting`);
            handleRerouting(currentLocation);
          }
        }
      }
    });
  ...
  }, [path, obstacles, alternativePaths, simulationActive, currentLocation]);
